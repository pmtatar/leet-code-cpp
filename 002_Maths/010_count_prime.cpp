...
public:
  ...
  Initially, all numbers are marked as prime except 0 and 1. We iteratively mark multiples of each prime starting from 2.

  Optimization:
  For a given prime i, we start marking multiples from i * i, because any smaller multiple (i * k where k < i) would have already
  been marked by a smaller prime.

  Time Complexity:  O(n log log n). Space Complexity: O(n).

  Note:
  std::vector<bool> is a specialized C++ template that stores bits in a packed format as bitset instead of individual bool objects,
  ...
  int primeUptoN_Sieve(int n) {
    ...
    for (int i = 2; i * i <= n; ++i) {
        ...
        if (isPrime[i]) {
            ...
            // Start marking from i*i since smaller multiples would have already been processed
